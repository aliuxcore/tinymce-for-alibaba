	...
	function Plugin(editor) {
		...
		function tableDialog() {
			...
			editor.windowManager.open({
				...
				items: {
					...
					items: [
						createNewTable ? {label: '列', name: 'cols', disabled: true} : null,
						createNewTable ? {label: '行', name: 'rows', disabled: true} : null,
						{label: '宽度', name: 'width'},
						{label: '高度', name: 'height'},
						{label: '单元格间距', name: 'cellspacing'},
						{label: '单元格边距', name: 'cellpadding'},
						{label: '边框', name: 'border'},
						{label: '标题', name: 'caption', type: 'checkbox'},
						{
							label: '对齐',
							minWidth: 90,
							name: 'align',
		...
		function mergeDialog(grid, cell) {
			editor.windowManager.open({
				title: "合并单元格",
				body: [
					{label: '列', name: 'cols', type: 'textbox', size: 10},
					{label: '行', name: 'rows', type: 'textbox', size: 10}
				],
				onsubmit: function() {
...
//						},
		...
		function insertTable(cols, rows) {
			...
			html = '<table cellpadding="2" cellspacing="0" style="border-collapse: collapse;" border="1"><tbody>';
		...
		editor.addMenuItem('cell', {
			...
			context: 'table',
			menu: [
				{text: '单元格属性', onclick: cmd('mceTableCellProps'), onPostRender: postRenderCell},
				{text: '合并单元格', onclick: cmd('mceTableMergeCells'), onPostRender: postRenderCell},
				{text: '分割单元格', onclick: cmd('mceTableSplitCells'), onPostRender: postRenderCell}
			]
		});
		...
		editor.addMenuItem('row', {
			...
			context: 'table',
			menu: [
				{text: '在上方插入行', onclick: cmd('mceTableInsertRowBefore'), onPostRender: postRenderCell},
				{text: '在下方插入行', onclick: cmd('mceTableInsertRowAfter'), onPostRender: postRenderCell},
				{text: '删除行', onclick: cmd('mceTableDeleteRow'), onPostRender: postRenderCell},
				{text: '行属性', onclick: cmd('mceTableRowProps'), onPostRender: postRenderCell},
				{text: '-'},
				{text: '剪切行', onclick: cmd('mceTableCutRow'), onPostRender: postRenderCell},
				{text: '复制行', onclick: cmd('mceTableCopyRow'), onPostRender: postRenderCell},
				{text: '在上方粘贴行', onclick: cmd('mceTablePasteRowBefore'), onPostRender: postRenderCell},
				{text: '在下方粘贴行', onclick: cmd('mceTablePasteRowAfter'), onPostRender: postRenderCell}
			]
		});
		...
		editor.addMenuItem('column', {
			...
			context: 'table',
			menu: [
				{text: '在左边插入列', onclick: cmd('mceTableInsertColBefore'), onPostRender: postRenderCell},
				{text: '在右边插入列', onclick: cmd('mceTableInsertColAfter'), onPostRender: postRenderCell},
				{text: '删除列', onclick: cmd('mceTableDeleteCol'), onPostRender: postRenderCell}
			]
		});
