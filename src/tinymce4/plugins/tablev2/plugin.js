...
define("tinymce/tableplugin/Plugin", [
    ...
    var each = Tools.each, W = window, D = document, aym = W.aym, cnst = aym.cnst, util = aym.util, env = aym.env;
	...
	function Plugin(editor) {
		...
		function tableDialog() {
            ...
            var sValues = dom.getTableParam(tableElm);
            var d = new aym.control.ui.TablePropertyDialog(util.id());
            d.onForShow(cnst.EVENT.SUCCESS,
                function (oValues) {
                    editor.undoManager.transact(function () {
                        dom.setTableParam(oValues, tableElm);
		...
		function mergeDialog(grid, cell) {
            ...
            var d = new aym.control.ui.TableRowColumnDialog(util.id());
            d.onForShow(cnst.EVENT.SUCCESS,
                function (oValues) {
                    editor.undoManager.transact(function() {
                        grid.merge(cell, oValues.column, oValues.row);
		...
		function cellDialog() {
            ...
            var data = dom.getTableSize(cellElm);
            data.alignment = dom.getTextAlign(cellElm);

            var d = new aym.control.ui.TableCellPropertyDialog(util.id());
            d.onForShow(cnst.EVENT.SUCCESS,
                function (oValues) {
                    editor.undoManager.transact(function () {
                        each(cells, function (cellElm) {
                            dom.setStyle(cells, "width", oValues.width);
                            dom.setStyle(cells, "height", oValues.height);

                            if (oValues.alignment == "default") {
                                dom.setStyle(cells, "text-align", null);
                            } else {
                                dom.setStyle(cells, "text-align", oValues.alignment);
                            }
                        });
		...
		function rowDialog() {
            ...
            var data = dom.getTableSize(rowElm);
            data.alignment = dom.getTextAlign(rowElm);

            var d = new aym.control.ui.TableRowPropertyDialog(util.id());
            d.onForShow(cnst.EVENT.SUCCESS,
                function (oValues) {
                    ...
                    editor.undoManager.transact(function () {
                        each(rows, function (rowElm) {
                            ...
                            dom.setStyle(rowElm, "height", oValues.height);

                            if (oValues.alignment == "default") {
                                dom.setStyle(rowElm, "text-align", null);
                            } else {
                                dom.setStyle(rowElm, "text-align", oValues.alignment);
                            }
		...
		function insertTable(cols, rows) {
            ...
            html = '<table cellpadding="2" cellspacing="0" style="border-collapse: collapse;border-width: 1px;border-style: solid;border-color: #666666;"><tbody>';

			for (y = 0; y < rows; y++) {
				html += '<tr>';

				for (x = 0; x < cols; x++) {
					html += '<td style="border-width: 1px;border-style: solid;border-color: #666666;">' + (Env.ie ? " " : '<br>') + '</td>';
				}
		...
		function generateTableGrid() {
			...
			for (var y = 0; y < 10; y++) {
				...
				for (var x = 0; x < 10; x++) {
					html += '<td><a style="cursor: pointer;" href="#" data-mce-index="' + x + ',' + y + '"></a></td>';
				}
		...
		editor.addMenuItem('inserttable', {
			text: $M("editor.menu.title.table.insert"),
			context: 'table',
			onhide: function() {
		...
		editor.addMenuItem('tableprops', {
			text: $M("editor.menu.title.table.property"),
			context: 'table',
		...
		editor.addMenuItem('deletetable', {
			text: $M("editor.menu.title.table.delete"),
			context: 'table',
			onPostRender: postRender,
			cmd: 'mceTableDelete'
		});
        ...
        editor.addMenuItem('cell', {
            separator : 'before',
            text : $M("editor.menu.title.table.cell"),
            context : 'table',
            menu : [
                {text : $M("editor.menu.title.table.cell.property"), onclick : cmd('mceTableCellProps'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.cell.merge"), onclick : cmd('mceTableMergeCells'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.cell.split"), onclick : cmd('mceTableSplitCells'), onPostRender : postRenderCell}
            ]
        });

        editor.addMenuItem('row', {
            text : $M("editor.menu.title.table.row"),
            context : 'table',
            menu : [
                {text : $M("editor.menu.title.table.row.insert.before"), onclick : cmd('mceTableInsertRowBefore'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.row.insert.after"), onclick : cmd('mceTableInsertRowAfter'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.row.delete"), onclick : cmd('mceTableDeleteRow'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.row.property"), onclick : cmd('mceTableRowProps'), onPostRender : postRenderCell},
                {text : '-'},
                {text : $M("editor.menu.title.table.row.cut"), onclick : cmd('mceTableCutRow'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.row.copy"), onclick : cmd('mceTableCopyRow'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.row.paste.before"), onclick : cmd('mceTablePasteRowBefore'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.row.paste.after"), onclick : cmd('mceTablePasteRowAfter'), onPostRender : postRenderCell}
            ]
        });

		editor.addMenuItem('column', {
            text : $M("editor.menu.title.table.column"),
            context : 'table',
            menu : [
                {text : $M("editor.menu.title.table.column.insert.before"), onclick : cmd('mceTableInsertColBefore'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.column.insert.after"), onclick : cmd('mceTableInsertColAfter'), onPostRender : postRenderCell},
                {text : $M("editor.menu.title.table.column.delete"), onclick : cmd('mceTableDeleteCol'), onPostRender : postRenderCell}
            ]
        });
        ...
        each("inserttable tableprops deletetable | cell row column".split(' '), function (name) {
            if (name == '|') {
                ...
                var tableItem = editor.menuItems[name];
                menuItems.push(tableItem);
            }
        });

        editor.addButton("table", {
            type : "extmenubutton",
            title : $M("editor.menu.title.table"),
            menu : menuItems
        });
	...
	PluginManager.add('tablev2', Plugin);
});
