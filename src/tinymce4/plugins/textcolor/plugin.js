...
tinymce.PluginManager.add('textcolor', function(editor) {
		...
		for (y = 0; y < rows; y++) {
			...
			for (x = 0; x < cols; x++) {
				...
				if (i > last) {
                    ...
                    color = colors[i];
                    var id = ctrl._id + '-' + i, style = color ? 'background-color: #' + color.color + ';' : '';
                    html += (
                        '<td><div id="' + id + '" data-mce-color="' + color.color + '" role="option" tabIndex="-1"' +
                            ' style="' + style + '"></div></td>');
                }
            }
	...
	editor.addButton('forecolor', {
		type: 'extcolorbutton',
		tooltip: editor.getSettingsMessage("button.text.color"),
		popoverAlign: 'bc-tl',
		selectcmd: 'ForeColor',
		panel: {
	...
	editor.addButton('backcolor', {
		type: 'extcolorbutton',
		tooltip: editor.getSettingsMessage("button.background.color"),
		popoverAlign: 'bc-tl',
