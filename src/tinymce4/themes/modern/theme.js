...
tinymce.ThemeManager.add('modern', function(editor) {
	...
	function createToolbars() {
				...
				if (item == "|") {
					...
					if (Factory.has(item)) {
						...
						if (editor.buttons[item]) {
							...
							item.type = item.type || 'extbutton';

							if (settings.toolbar_items_size) {
								item.size = settings.toolbar_items_size;
							}

                            if (itemName == "fullscreen") {
                                item.subtype = "ext-btn-right";
                            }
		...
		// Generate toolbar<n>
		for (var i = 1; i < 10; i++) {
			if (!addToolbar(settings["toolbar_" + i])) {
				break;
			}
		}
...
//			addAccessibilityKeys(panel);
	...
	function renderIframeUI(args) {
        ...
        var toolbar = createToolbars();
        toolbar[toolbar.length - 1].style = "display:none;"

		// Basic UI layout
        var bw = settings.border_width;
		panel = self.panel = Factory.create({
			type: 'panel',
			classes: 'tinymce',
...
//		addAccessibilityKeys(panel);
	...
	self.renderUI = function(args) {
        var cssFiles = settings.cssFiles;
        if (cssFiles) {
            for (var i = 0, len = cssFiles.length; i < len; i++) {
                tinymce.DOM.loadCSS(tinymce.baseURL + "/" + cssFiles[i]);
            }
        }
