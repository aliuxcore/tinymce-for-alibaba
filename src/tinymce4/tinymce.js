...
try {
	...
	var nodeType = arr[preferredDoc.childNodes.length].nodeType;
} catch ( e ) {
...
Expr = Sizzle.selectors = {
	...
	pseudos: {
		...
		"selected": function( elem ) {
			...
			if ( elem.parentNode ) {
                var selectedIndex = elem.parentNode.selectedIndex;
			}
...
define("tinymce/util/Tools", [], function() {
    ...
    function isTrue(s) {
        return s === '1' || s === 1 || s === true || (typeof(s) == "string" && s.toLowerCase() === 'true');
    }
	...
	return {
		...
		resolve: resolve,
		explode: explode,
        isSupportFileReader: isSupportFileReader,
        isTrue: isTrue,
...
define("tinymce/util/StringUtils", [], function () {
    ...
    function replaceAll(str, s1, s2) {
        return str ? str.replace(new RegExp(s1, "gm"), s2) : "";
    }

    function contains(str, s) {
        return str ? str.indexOf(s) !== -1 : false;
    }

    function startsWithIgnoreCase(str, s) {
        if (s == null || s == "" || str.length == 0 || s.length > str.length) {
            return false;
        }
        return str.substring(0, s.length).toLowerCase() == s.toLowerCase();
    }
...
define("tinymce/Env", [], function() {
	...
	var opera, webkit, ie, ie11, gecko, mac, iDevice, win6;
    ...
    win6 = /Windows\s+NT\s+6/.test(userAgent);
	...
	return {
        ...
        /**
         * Constant that is true if the os is Win7 or Win Vista.
         *
         * @property win6
         * @type Boolean
         * @final
...
define("tinymce/dom/DOMUtils", [
...
], function(Sizzle, Styles, EventUtils, TreeWalker, Range, Entities, Env, Tools, StringUtils) {
	// Shorten names
	...
	DOMUtils.prototype = {
        ...
        setTableParam : function (oValues, oTableNode) {
            if (oTableNode) {
                if (oValues.cellPadding != undefined) {
                    this.setAttrib(oTableNode, "cellpadding", oValues.cellPadding);
                }

                if (oValues.cellSpacing != undefined) {
                    this.setAttrib(oTableNode, "cellspacing", oValues.cellSpacing);
                }

                var sAlignment = oValues.alignment;
                if (sAlignment != undefined) {
                    if (sAlignment == "left") {
                        this.setStyles(oTableNode, {
                            "float" : "left",
                            "margin-left" : 0,
                            "margin-right" : 0
                        });
                ...
                var collapse = oValues.collapse;
                if (collapse != undefined) {
                    if (Tools.isTrue(collapse)) {
                        this.setStyle(oTableNode, "border-collapse", "collapse");
                    } else {
                        this.setStyle(oTableNode, "border-collapse", "separate");
                    }
                }

                if (oValues.width != undefined) {
                    this.setStyle(oTableNode, "width", oValues.width);
                }

                if (oValues.height != undefined) {
                    this.setStyle(oTableNode, "height", oValues.height);
                }

                if (oValues.border != undefined) {
                    var nBorderWidth = parseInt(oValues.border);
                    nBorderWidth = nBorderWidth || 0;
                    this.setTableBorder(oTableNode, nBorderWidth);
                }
            }
        },
        contentFilterRegExp : new RegExp([
            '<span[^>]+data-mce-bogus[^>]+>[\u200B\uFEFF]+<\\/span>',
            '<div[^>]+data-mce-bogus[^>]+><\\/div>',
            '\\s?data-mce-selected="[^"]+"'
        ].join('|'), 'gi'),
        contentFilter : function (sContent) {
            return sContent ? sContent.replace(this.contentFilterRegExp, '') : "";
        },
        setTableBorder : function (table, borderWidth, keepValue) {
            if (table) {
                var self = this;

                var setBorder = function (node, bw) {
                    var borderWidth = bw + "px", borderColor = "#666666", borderStyle = "solid";

                    if (keepValue) {
                        borderColor = this.getStyle(node, "border-color", true) || borderColor;
                    }

                    self.setAttrib(node, "border", null);
                ...
                setBorder.call(self, table, borderWidth);

                var nodes = self.select("td", table);
                for (var i = 0, l = nodes.length; i < l; i++) {
                    setBorder.call(self, nodes[i], borderWidth);
                }
            }
        },
        getTableBorderWidth : function (oDomNode) {
            var nBorderWidth = 0;
            if (oDomNode) {
                nBorderWidth = this.getStyle(oDomNode, "border-width");

                if (nBorderWidth) {
                    nBorderWidth = parseInt(nBorderWidth);
                }

                if ((!nBorderWidth) && nBorderWidth !== 0) {
                    nBorderWidth = this.getAttrib(oDomNode, "border");
                    if (nBorderWidth) {
                        nBorderWidth = parseInt(nBorderWidth);
                    }
                }
                ...
                nBorderWidth = nBorderWidth || 0;
                this.setTableBorder(oDomNode, nBorderWidth);
            }
            return nBorderWidth;
        },
        getTableSize : function (oTableNode) {
            var nBorderWidth = 0;
            if (oTableNode) {
                var styles = this.parseStyle(oTableNode.style.cssText);
                var sWidth = styles.width, sHeight = styles.height;
                if (!sWidth) {
                    sWidth = this.getAttrib(oTableNode, "width");
                    if (sWidth) {
                        sWidth += "px";
                    } else {
                        sWidth = "auto";
                ...
                if (!sHeight) {
                    sHeight = this.getAttrib(oTableNode, "height");
                    if (sHeight) {
                        sHeight += "px";
                    } else {
                        sHeight = "auto";
                ...
                sWidth = StringUtils.replaceAll(sWidth, "pt", "px");
                sHeight = StringUtils.replaceAll(sHeight, "pt", "px");
                this.setAttrib(oTableNode, "width", null);
                this.setAttrib(oTableNode, "height", null);
                this.setStyle(oTableNode, "width", sWidth);
                this.setStyle(oTableNode, "height", sHeight);
            }
        ...
        getTextAlign : function (oDomNode) {
            var sTextAlign;
            if (oDomNode) {
                sTextAlign = this.getStyle(oDomNode, 'text-align');
                sTextAlign = sTextAlign || "default";
            }
            return sTextAlign;
        },
        getTableParam : function (oTableNode) {
            if (oTableNode) {
                var retObj = this.getTableSize(oTableNode) || {};
                retObj.border = this.getTableBorderWidth(oTableNode) + "";
                retObj.cellPadding = this.getAttrib(oTableNode, "cellpadding") || "0";
                retObj.cellSpacing = this.getAttrib(oTableNode, "cellspacing") || "0";

                var sCollapse = this.getStyle(oTableNode, "border-collapse");
                if (sCollapse == "collapse") {
                    retObj.collapse = "1";
                } else {
                    retObj.collapse = "0";
                }
                ...
                var styles = this.parseStyle(oTableNode.style.cssText);
                var sAlignment, sFloat = this.getStyle(oTableNode, "float");
                ...
                if (sFloat == "left") {
                    ...
                    if (styles["margin-left"] == "auto" && styles["margin-right"] == "auto") {
                        sAlignment = "center";
                    } else {
                        sAlignment = "default";
                    }
                }
                retObj.alignment = sAlignment;
                return retObj;
            }
        },
        setSelectable : function (oDomNode, value) {
            if (!oDomNode) return;

            var value = !!value;

            if (Env.ie) {
                this.setAttribs(oDomNode, value ? {"unselectable" : "off", "onselectstart" : "return true"} : {"unselectable" : "on", "onselectstart" : "return false"});
            } else {
                this[value ? "removeClass" : "addClass"](oDomNode, "unselectable");
            }
        },
		fixDoc: function(doc) {
		...
		this.loadScripts = function(scripts, callback, scope) {
			...
			loadScripts = function() {
				...
				each(loadingScripts, function(url) {
                    if (window.tinymceSkipScriptLoad) {
                        states[url] = LOADED;
                    }
...
define("tinymce/UndoManager", [
    ...
    "tinymce/dom/DOMUtils",
	"tinymce/util/Tools"
], function(VK, Env, DOMUtils, Tools) {
	var trim = Tools.trim, DOM = DOMUtils.DOM, trimContentRegExp;
	...
	return function(editor) {
		...
		function getContent() {
            var content = editor.getContent({format: 'raw', no_events: 1, no_convert: 1});
			return DOM.contentFilter(content);
		}
        ...
        var lastKeyAddUndoTime;
        editor.on('KeyUp', function (e) {
            var kc = e.keyCode, allowContinue = false;
            ...
            if (!e.altKey) {
                if (e.ctrlKey) {
                    ...
                    if ((kc >= 65 && kc <= 90) || kc == 192 || kc == 32 || kc == 8 || kc == 46 || (kc >= 96 && kc <= 111)
                        || (kc >= 219 && kc <= 222) || (kc >= 48 && kc <= 57) || (kc >= 186 && kc <= 191))
                        allowContinue = true;
                }
            }
            ...
            self.typing = true;
            var now = lastKeyAddUndoTime = Tools.getTimestamp();

            setTimeout(function () {
                if (now == lastKeyAddUndoTime) {
                    addNonTypingUndoLevel();
                    self.beforeChange();
                }
            }, 600);
        ...
        editor.on('KeyUp', function (e) {
            ...
            if ((keyCode >= 37 && keyCode <= 40) && (!VK.modifierPressed(e))) {
                var now = lastKeyUpTime = Tools.getTimestamp();

                setTimeout(function () {
                    if (now == lastKeyUpTime) {
                        editor.nodeChanged();
                    }
                }, 300);
            }
        ...
        editor.on('AddUndo Undo Redo MouseUp', function (e) {
		...
		self = {
			...
			add: function(level) {
				...
				if (lastLevel && lastLevel.content.length == level.content.length) {
					return null;
				}
			...
			undo: function() {
				...
				if (index > 0) {
					...
					editor.setContent(level.content, {format: 'raw', no_convert: 1});
	...
	return function(editor) {
		...
		function handleEnterKey(evt) {
			...
			function handleEmptyListItem() {
                ...
                if (newBlock) {
                    var defaultStyleFun = settings.default_style_fun;
                    if (defaultStyleFun) {
                        var defaultStyle = defaultStyleFun();
                        Tools.isEmptyObject(defaultStyle) || dom.setStyles(newBlock, defaultStyle);
                    }
                }
...
define("tinymce/ForceBlocks", ["tinymce/util/Tools"], function(Tools) {
	return function(editor) {
		var settings = editor.settings, dom = editor.dom, selection = editor.selection;
		var schema = editor.schema, blockElements = schema.getBlockElements();

        function beforeAddRootBlocks() {
            var rootNode = editor.getBody();
            if (rootNode.childNodes.length > 0 && rootNode.firstChild.nodeType != 1) {
                return;
            }

            var children = rootNode.children;
            if (children.length == 0) {
                dom.setHTML(rootNode, "<br>");
            } else {
                var firstChild = children[0], nodeName = firstChild.nodeName.toLowerCase();
                if (nodeName != "br") {
                    var innerHTML = firstChild.innerHTML;
                    if (!innerHTML || Tools.trim(innerHTML) == "") {
                        dom.replace(dom.create('br'), firstChild);
                    }
                }
            }
        }
			...
			while (node) {
				...
				if (((node.nodeType === 3 || (node.nodeType == 1 && !blockElements[node.nodeName]))) &&
					...
					if (!rootBlockNode) {
                        ...
                        editor.fire("AddRootBlocks");

                        var defaultStyleFun = settings.default_style_fun;
                        if (defaultStyleFun) {
                            var defaultStyle = defaultStyleFun();
                            Tools.isEmptyObject(defaultStyle) || dom.setStyles(rootBlockNode, defaultStyle);
                        }
...
define("tinymce/ui/DomUtils", [
	...
	return {
        ...
        setSelectable : function (elm, value) {
            return DOMUtils.DOM.setSelectable(elm, value);
        },
...
//				}
		...
		postRender: function() {
            ...
            if (settings.unselectable) {
                DomUtils.setSelectable(self.getEl(), false);
            }
		...
		init: function(settings) {
			...
			if (settings.autohide) {
				if (!documentClickHandler) {
					...
					DomUtils.on(document, 'mousedown', documentClickHandler);
				}
	...
	return function(editor) {
		...
		self.open = function(args, params) {
			...
			if (!args.url && !args.buttons) {
				args.buttons = [
					{text: editor.getSettingsMessage("button.ok"), subtype: 'primary', onclick: function() {
						win.find('form')[0].submit();
						win.close();
					}},

					{text: editor.getSettingsMessage("button.cancel"), onclick: function() {
						win.close();
					}}
				];
			}
...
define("tinymce/util/Quirks", [
	...
	return function(editor) {
		...
		function emptyEditorWhenDeleting() {
            ...
            //this function is too slow ,so disable
//			editor.on('keydown', function(e) {
...
//                        var start = new Date();
                        var ret = callback.call(self, args);
//                        var end = new Date();
//                        if ((end - start) > 500) {
//                            alert(end - start);
//                            alert(callback);
...
define("tinymce/Editor", [
	...
	Editor.prototype = {
		...
		init: function() {
            ...
            DOM.setAttrib(n, "editor_id", self.id);
            var iframeCls = settings.iframe_class;
            iframeCls && DOM.addClass(n, iframeCls);
...
//			});
         ...
         * Returns i18n.
         *
         * @method getSettingsMessage
         */
        getSettingsMessage : function (name) {
            var i18nMessages = this.settings.i18n_messages || {};
            var value = i18nMessages[name];
            return value || name;
        },
		...
		setContent: function(content, args) {
            ...
            if (!args.no_convert) {
                self.fire('ConvertSetContent', args);
            }
			...
			if (content.length === 0 || /^\s+$/.test(content)) {
				...
				if (forcedRootBlockName && self.schema.isValidChild(body.nodeName.toLowerCase(), forcedRootBlockName.toLowerCase())) {
                    ...
                    var renderEmptyFun = self.settings.render_empty_fun;
                    if (renderEmptyFun) {
                        content = renderEmptyFun();
                    } else {
...
//			}
            ...
            content = DOM.contentFilter(content);
            args.content = content;
            ...
            if (!args.no_convert) {
                self.fire('ConvertGetContent', args);
            }
...
define("tinymce/EditorManager", [
	...
	var EditorManager = {
        ...
        getHelperIframeWin : function () {
            var helperIframeWin = this.helperIframeWin;
            if (!helperIframeWin) {
                var nodes = DOM.select("#tinymce_helper iframe", document);
                if (nodes && nodes.length > 0) {
                    var helperIframe = nodes[0];
                    helperIframeWin = helperIframe.contentWindow;
                    this.helperIframeWin = helperIframeWin;
                }
            }
            return helperIframeWin;
        },
        getHelperIframeBody : function () {
            var helperIframeBody = this.helperIframeBody;
            if (!helperIframeBody) {
                var helperIframeWin = this.getHelperIframeWin();
                if (helperIframeWin) {
                    helperIframeBody = helperIframeWin.document.body;
                    this.helperIframeBody = helperIframeBody;
                }
            }
            return helperIframeBody;
        }
	};
            ...
            renderHtml : function () {
                ...
                return (
                    '<div id="' + id + '" class="' + self.classes() + '">' +
                        '<div role="presentation" class="' + prefix + 'separate-inner"></div>' +
                        '</div>'
                    );
            }
        });
...
define("tinymce/ui/ExtButton", [
    ...
    return Button.extend({
        Defaults: {
            classes: "widget ext-btn ext-btn-small",
            role: "extbutton"
        },
        renderHtml: function () {
            var self = this, id = self._id, prefix = self.classPrefix;
            var settings = self.settings, icon = settings.icon, image = '', tooltip = settings.tooltip;
            ...
            icon = self.settings.icon ? prefix + 'ext-ico ' + prefix + 'ext-i-' + icon : '';

            return (
                '<div id="' + id + '" class="' + self.classes() + '" ' + (tooltip ? ('title="' + tooltip + '"') : '') + ' tabindex="-1">' +
                    '<div class="' + prefix + 'ext-btn-inner" role="presentation" tabindex="-1">' +
                    (icon ? '<div class="' + icon + '"' + image + '/></div>' : '') +
                    (self._text ? (icon ? ' ' : '') + self.encode(self._text) : '') +
                    '</div>' +
                    '</div>'
                );
...
define("tinymce/ui/PanelButton", [
	...
	return Button.extend({
		...
		showPanel: function() {
			...
			if (!self.panel) {
				...
				self.panel.reflow().moveRel(self.getEl(), ['bl-tl', 'tl-bl']);
			} else {
				self.panel.show();
                self.panel.moveRel(self.getEl(), ['bl-tl', 'tl-bl']);
			}
    ...
    define("tinymce/ui/ExtPanelButton", [
...
define("tinymce/ui/ColorButton", [
    ...
    define("tinymce/ui/ExtColorButton", [
        "tinymce/ui/ColorButton"
    ], function (ColorButton) {
        "use strict";

        return ColorButton.extend({
            Defaults: {
                classes: "widget ext-btn ext-btn-small ext-colorbutton",
                role: "extcolorbutton"
            },
            init: function (settings) {
                this._super(settings);
            ...
            renderHtml: function () {
                var self = this, id = self._id, prefix = self.classPrefix;
                var icon = self.settings.icon ? prefix + 'ext-ico ' + prefix + 'ext-i-' + self.settings.icon : '';
                var image = self.settings.image ? ' style="background-image: url(\'' + self.settings.image + '\')"' : '';
                var tooltip = self.settings.tooltip;

                return (
                    '<div id="' + id + '" class="' + self.classes() + '"  ' + (tooltip ? ('title="' + tooltip + '"') : '') + ' tabindex="-1">' +
                        '<div role="presentation" class="' + prefix + 'ext-btn-inner" tabindex="-1">' +
                        (icon ? '<div  class="' + icon + '"' + image + '></div>' : '') +
                        '<div id="' + id + '-preview" class="' + prefix + 'preview"></div>' +
                        (self._text ? (icon ? ' ' : '') + self.encode(self._text) : '') +
                        '</div><div class="' + prefix + 'open" hidefocus tabindex="-1">' +
                        ' <div class="' + prefix + 'ext-caret"></div>' +
                        '</div>' +
                        '</div>'
                    );
            }
...
define("tinymce/ui/FormatControls", [
    ...
    "tinymce/dom/DOMUtils",
	"tinymce/Env"
], function(Control, Widget, FloatPanel, Tools, StringUtils, EditorManager, DOMUtils, Env) {
	var each = Tools.each, DOM = DOMUtils.DOM;
	...
	function registerControls(editor) {
        ...
        function getFontStyle(e, formatName) {
            if (e && e.element && formatName) {
                var element = e.element, styleName;
                ...
                if (styleName) {
                    ...
                    if (element.nodeType == 1) {
                        try {
                            styleValue = DOM.getStyle(element, styleName, true);
                        } catch (e) {
                        }
                    }

                    return styleValue ? Tools.trim(styleValue).replace(/'|"/g,"").toLowerCase() : styleValue;
                }
            }
            return null;
        }

        function getFirstFontFamily(s) {
            var firstEN, firstZHCN;

            if (s) {
                var fontList = s.split(",");
                for (var i = 0, len = fontList.length; i < len; i++) {
                    var font = Tools.trim(fontList[i]);
                    if (StringUtils.hasChinese(font) || font == "simsun" || font == "simhei" || font == "microsoft yahei"
                        || font == "lisu" || font == "youyuan") {

                        firstZHCN = firstZHCN || font;
                    } else {
                        firstEN = firstEN || font;
                    }
                }
            }
        ...
        var systemDefaultFontFamily = undefined;

        function isSystemDefaultFontFamily(value) {
            if (value) {
                if (!systemDefaultFontFamily) {
                    var nodes = DOM.select(">html head", editor.getDoc());
                    if (nodes.length > 0) {
                        var styleNodes = DOM.select(">style", nodes[0]);
                        if (styleNodes.length > 0) {
                            systemDefaultFontFamily = Tools.trim(DOM.getStyle(editor.getBody(), "font-family", 1)).toLowerCase();
                        }
                    }
                }

                return value == systemDefaultFontFamily;
            }
            return false;
        }
        ...
        function parseFontFamily(value, e) {
            if (value) {
        ...
        function createListBoxChangeHandler(items, formatName) {
			return function() {
                ...
                editor.on('nodeChange', function (e) {
                    //not support formatselect , because alimail has not use formatselect
                    if (formatName == "fontname" || formatName == "fontsize") {
                        var value = getFontStyle(e, formatName);

                        if (formatName == "fontsize") {
                            value = formatFontSize(value);

                        } else if (formatName == "fontname") {
                            if (value) {
                                if (isSystemDefaultFontFamily(value)) {
                                    if (editor.settings.show_system_default_font && Env.win6) {
                                        value = parseFontFamily(value, e);
                                    } else {
                                        value = editor.getSettingsMessage("default.font");
                                    }
		...
		each({
			bold: editor.getSettingsMessage("button.bold"),
			italic: editor.getSettingsMessage("button.italic"),
			underline: editor.getSettingsMessage("button.underline"),
			strikethrough: editor.getSettingsMessage("button.strikethrough"),
			subscript: 'Subscript',
		...
		each({
			outdent: [editor.getSettingsMessage("button.outdent"), 'Outdent'],
			indent: [editor.getSettingsMessage("button.indent"), 'Indent'],
			cut: ['Cut', 'Cut'],
			copy: ['Copy', 'Copy'],
			paste: ['Paste', 'Paste'],
			help: ['Help', 'mceHelp'],
			selectall: ['Select all', 'SelectAll'],
			hr: [editor.getSettingsMessage("button.horizontal.line"), 'InsertHorizontalRule'],
			removeformat: [editor.getSettingsMessage("button.remove.format"), 'RemoveFormat'],
...
//			bullist: ['Bulleted list', 'InsertUnorderedList'],
			...
			alignleft: [editor.getSettingsMessage("button.align.left"), 'JustifyLeft'],
			aligncenter: [editor.getSettingsMessage("button.align.center"), 'JustifyCenter'],
			alignright: [editor.getSettingsMessage("button.align.right"), 'JustifyRight'],
			alignjustify: [editor.getSettingsMessage("button.align.justify"), 'JustifyFull']
		}, function(item, name) {
			editor.addButton(name, {
				tooltip: item[0],
				cmd: item[1],
		...
		editor.addButton('undo', {
			tooltip: editor.getSettingsMessage("button.undo"),
			onPostRender: toggleUndoState,
			cmd: 'undo'
		});

		editor.addButton('redo', {
			tooltip: editor.getSettingsMessage("button.redo"),
		...
		editor.addButton('fontselect', function() {
            ...
            var items = [], fonts = createFormats(editor.getSettingsMessage("font.family.list"));
			...
			each(fonts, function(font) {
                ...
                items.push({
                    ...
                    textStyle : StringUtils.contains(value, "@") ? "" : 'font-family:' + value
                });
			});

			return {
				type: 'extlistbox',
                text: editor.getSettingsMessage("button.font.family"),
                menuConfig : {
                    maxHeight : 200,
                    minWidth : 175,
                    menuCls : editor.getSettings().menu_class
                },
                subtype : "selecor mce-selecor-left",
				values: items,
		...
		editor.addButton('fontsizeselect', function() {
            ...
            each(editor.getSettingsMessage("font.size.list").split(';'), function (item) {
                items.push({
                    text : item,
                    value : item,
                    textStyle : "font-size:" + item
                });
            });

			return {
                type: 'extlistbox',
                text : editor.getSettingsMessage("button.font.size"),
                subtype: "selecor",
                menuConfig : {
                    maxHeight : 200,
                    minWidth : 160,
...
define("tinymce/ui/MenuButton", [
	...
	var MenuButton = Button.extend({
		...
		showMenu: function() {
			...
			if (!self.menu) {
                ...
                var menuConfig = settings.menuConfig;
                if (menuConfig) {
                    menu.width = menuConfig.width;
                    menu.height = menuConfig.height;
                    menu.maxWidth = menuConfig.maxWidth;
                    menu.maxHeight = menuConfig.maxHeight;
                    menu.minWidth = menuConfig.minWidth;
                    menu.minHeight = menuConfig.minHeight;
                    menu.menuCls = menuConfig.menuCls;
                }
            ...
            settings.beforeShowMenu && settings.beforeShowMenu.call(self);
			self.menu.show();
            var oLayoutRect = self.layoutRect();
            self.menu.layoutRect({w : oLayoutRect.w, h : oLayoutRect.h});
    ...
    define("tinymce/ui/ExtMenuButton", [
        "tinymce/ui/MenuBar",
        "tinymce/ui/MenuButton"
    ], function (MenuBar, MenuButton) {
        "use strict";

        var ExtMenuButton = MenuButton.extend({
            Defaults: {
                classes: "widget ext-btn ext-btn-small ext-menubtn",
                role: "extmenubutton"
            },
            renderHtml: function () {
                var self = this, id = self._id, prefix = self.classPrefix;
                ...
                self.aria('role', self.parent() instanceof MenuBar ? 'menuitem' : 'extbutton');

                var html = [];
                html.push('<div id="' + id + '" class="' + self.classes() + '"  ' + (tooltip ? ('title="' + tooltip + '"') : '') + ' tabindex="-1">');
                html.push('<div id="' + id + '-open" role="presentation" class="' + prefix + 'ext-btn-inner" tabindex="-1">');
                ...
                if (self._text) {
                    html.push('<div class="' + prefix + 'ext-btn-wrap ellipsis"><span class="text_middle">' + self.encode(self._text) + '</span>&#x200D;</div>');
                }
                html.push('</div>');
                html.push('<div class="' + prefix + 'open"><div class="' + prefix + 'ext-caret"></div></div>');
                html.push('</div>');
                return html.join("");
            }
        });
        return ExtMenuButton;
    });
...
define("tinymce/ui/ListBox", [
    ...
    define("tinymce/ui/ExtListBox", [
        "tinymce/util/StringUtils",
        "tinymce/ui/ExtMenuButton"
    ], function (StringUtils, ExtMenuButton) {
        "use strict";

        return ExtMenuButton.extend({
            /**
             * Constructs a instance with the specified settings.
             *
             * @constructor
            ...
            init: function (settings) {
                ...
                self.addClass('btn-listbox-small');
            ...
            value: function (value) {
                ...
                function activateByValue(menu, value) {
                    menu.items().each(function (ctrl) {
                        active = StringUtils.equalsIgnoreCase(ctrl.value(), value);
                ...
                if (typeof(value) != "undefined") {
                    if (self.menu) {
                        ...
                        menu = self.settings.menu;
                        for (i = 0; i < menu.length; i++) {
                            active = StringUtils.equalsIgnoreCase(menu[i].value, value);
                    ...
                    self.text(selectedText || (settings.alwaysShowValue ? (settings.alwaysShowValueFormat ? settings.alwaysShowValueFormat(value) : value) : undefined) || settings.text);
                }
	...
	return Widget.extend({
		...
		postRender: function() {
            ...
            if (textStyle) {
                ...
                if (textElm) {
                    textElm.style.cssText = textStyle;
                }
            }
...
define("tinymce/ui/Menu", [
	...
	"tinymce/util/Tools",
    "tinymce/dom/DOMUtils"
], function(FloatPanel, KeyboardNavigation, MenuItem, Tools, DomUtils) {
	"use strict";
	...
	var Menu = FloatPanel.extend({
		...
		repaint: function() {
            ...
            var settings = this.settings;
            var menuCls = settings.menuCls;
            menuCls && DOM.addClass(this.getEl('body'), menuCls);
...
define("tinymce/ui/ResizeHandle", [
	...
	return Widget.extend({
		...
		renderHtml: function() {
			...
			return (
				'<div id="' + self._id + '" class="' + self.classes() + '">' +
					'<i class="' + prefix + 'ext-ico ' + prefix + 'ext-i-resize"></i>' +
				'</div>'
			);
		},
...
define("tinymce/ui/SplitButton", [
	...
	return MenuButton.extend({
		...
		activeMenu: function(state) {
			...
			DOM.toggleClass(self.getEl(), self.classPrefix + 'active', state);
		},
    ...
    define("tinymce/ui/ExtSplitButton", [
        "tinymce/ui/SplitButton"
    ], function (SplitButton) {

        return SplitButton.extend({
            Defaults: {
                classes: "widget ext-btn ext-splitbtn ext-btn-small",
                role: "extsplitbutton"
            },
            renderHtml: function () {
                var self = this, id = self._id, prefix = self.classPrefix;
                ...
                return (
                    '<div id="' + id + '" class="' + self.classes() + '" ' + (tooltip ? ('title="' + tooltip + '"') : '') + ' >' +
                        '<div class="' + prefix + 'ext-btn-inner" hidefocus tabindex="-1">' +
                        (icon ? '<div class="' + icon + '"></div>' : '') +
                        (self._text ? (icon ? ' ' : '') + self._text : '') +
                        '</div>' +
                        '<div class="' + prefix + 'open" hidefocus tabindex="-1">' +
                        //(icon ? '<i class="' + icon + '"></i>' : '') +
                        (self._menuBtnText ? (icon ? ' ' : '') + self._menuBtnText : '') +
                        ' <div class="' + prefix + 'ext-caret"></div>' +
